...
import { card_data_newyork } from './data_comp';
...
function NewYork() {
  const [items, setItems] = useState(card_data_newyork);
  const [visible, setVisible] = useState(6);
  const [loading, setloading] = useState(false);
  const navigate = useNavigate();

  useEffect(() => {
    fetch(card_data_newyork)
      .then((res) => res.json())
        ...
        {items.slice(0, visible).map((item, idx) => {
          return (
            <Card
              ...
              // }}
              key={idx}
              style={{ width: '25rem', margin: '5vh 0', marginLeft: 'auto', marginRight: 'auto', background: '#F2F6FC', border: '1px white', boxShadow: 'rgba(100, 100, 111, 0.2) 0px 7px 29px 0px', textDecoration: 'none' }}>
